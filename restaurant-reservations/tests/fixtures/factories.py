...
class RestaurantFactory(factory.django.DjangoModelFactory):
    ...
    phone = factory.LazyAttribute(
        lambda _: f"+{random.randint(1, 999)}-{random.randint(100, 999)}-{random.randint(1000, 9999)}"
    )
...
class TableFactory(factory.django.DjangoModelFactory):
    ...
    capacity = factory.Faker("random_int", min=4, max=8)
...
class ReservationFactory(factory.django.DjangoModelFactory):
    ...
    table = factory.SubFactory(TableFactory, restaurant=factory.SelfAttribute('..restaurant'))
    reservation_date = factory.LazyFunction(
        lambda: timezone.now().date() + timedelta(days=1)
    )
    reservation_time = factory.Faker(
        "random_element",
        elements=[
            time(10, 0),
            time(12, 0),
    ...
    party_size = 2  # Valor fijo que sea compatible con la mayoría de mesas
