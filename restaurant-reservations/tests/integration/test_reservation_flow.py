...
class ReservationFlowIntegrationTest(TransactionTestCase):
    ...
    def test_complete_reservation_lifecycle(
        self, mock_reminder, mock_expire, mock_email
    ):
...
class SystemIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for restaurant in self.restaurants:
            restaurant_tables = [
                TableFactory(restaurant=restaurant, number=str(i)) for i in range(1, 4)
            ]
    ...
    def test_complex_reservation_scenario(self, mock_send_mail):
        ...
        # Verify no conflicts
        active_reservations = [
            r for r in reservations if r.status in ["pending", "confirmed"]
        ]
        self.assertEqual(len(active_reservations), 5)

        # Test querying reservations by various criteria
        restaurant1_reservations = Reservation.objects.filter(
        ...
        time_18_reservations = Reservation.objects.filter(
            reservation_time=time(18, 0), status__in=["pending", "confirmed"]
        )
