...
class APIIntegrationTest(TransactionTestCase):
    ...
    def test_end_to_end_reservation_creation(
        self, mock_expire, mock_availability, mock_lock
    ):
    ...
    def test_api_list_performance_with_many_reservations(self):
        ...
        for i in range(50):
            ...
            reservation = ReservationFactory(
                restaurant=self.restaurant, table=table, customer=customer
            )
...
class APISecurityIntegrationTest(TransactionTestCase):
    ...
    def test_malicious_json_payload(self):
        ...
        for payload in malicious_payloads:
            response = self.client.post(
                "/api/", json.dumps(payload), content_type="application/json"
            )
    ...
    def test_large_payload_handling(self):
        ...
        response = self.client.post(
            "/api/", json.dumps(large_data), content_type="application/json"
        )
