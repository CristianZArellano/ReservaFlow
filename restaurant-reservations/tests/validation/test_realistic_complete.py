...
class RealisticRedisLockTests(RealisticTestRunner):
    ...
    def test_concurrent_lock_contention(self):
        ...
        print(f"  {'✅' if success else '❌'} Contención manejada correctamente: {success}")
...
class RealisticCeleryTaskTests(RealisticTestRunner):
    ...
    def test_expire_reservation_task_success(self):
        ...
        task_result = next(
            (task for task in self.celery_queue if task['task_id'] == task_info['task_id']),
            None
        )
        ...
        success = (
            task_result and 
            task_result['status'] == 'SUCCESS' and
            task_result['result']['status'] == 'expired'
        )
    ...
    def test_expire_reservation_task_failure(self):
        ...
        success = (
            task_result and 
            task_result['status'] == 'SUCCESS' and  # Task ejecuta pero retorna error
            task_result['result']['error'] == 'not_found'
        )
    ...
    def test_email_confirmation_task(self):
        ...
        with patch('django.core.mail.send_mail') as mock_send_mail:
            ...
            # Ejecutar tarea
            task_info = self.simulate_celery_task(send_confirmation_email, str(reservation.id))
            ...
            success = (
                task_result and 
                task_result['status'] == 'SUCCESS' and
                task_result['result']['status'] == 'email_sent' and
                email_attempted
            )
...
class RealisticDatabaseConstraintTests(RealisticTestRunner):
    ...
    def test_concurrent_database_operations(self):
        ...
        total_attempts = len(successful_creations) + len(failed_creations)
...
def run_complete_realistic_tests():
    ...
    all_tests.extend([
        ("Celery Expire Success", celery_tests.test_expire_reservation_task_success),
        ("Celery Expire Failure", celery_tests.test_expire_reservation_task_failure),
