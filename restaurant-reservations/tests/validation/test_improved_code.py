...
class ImprovedCodeTest:
    ...
    def test_improved_redis_locks(self):
        ...
        avg_acquisition_time = (
            sum(lock_acquisition_times) / len(lock_acquisition_times)
            if lock_acquisition_times
            else 0
        )
        ...
        print(
            f"     ⏱️ Tiempo promedio de adquisición: {avg_acquisition_time * 1000:.1f}ms"
        )

        # Mostrar detalles de algunos resultados
        for result in results[:3]:
            print(
                f"     ✅ Cliente {result['client_id']}: {result['acquisition_time'] * 1000:.1f}ms, ext: {result['lock_extended']}"
            )
        ...
        improvement_detected = (
            successful_locks > 1
        )  # Al menos algunos deberían lograr el lock con retry
        ...
        print(
            f"  🔧 Mejoras implementadas funcionando: {'✅' if improvement_detected else '❌'}"
        )
    ...
    def test_improved_celery_retry(self):
        ...
        with patch("django.core.mail.send_mail") as mock_send_mail:
            ...
            mock_send_mail.side_effect = smtplib.SMTPServerDisconnected(
                "Connection lost"
            )
    ...
    def test_database_optimizations(self):
        ...
        with connection.cursor():
            ...
            for i in range(5):
                thread = threading.Thread(
                    target=create_reservation_with_lock, args=(i,)
                )
                threads.append(thread)
        ...
        print(
            f"  🔧 Optimizaciones funcionando: {'✅' if optimization_success else '❌'}"
        )
    ...
    def generate_improvement_report(self):
        ...
        print(
            f"  Porcentaje de éxito: {(successful_improvements / total_tests) * 100:.1f}%"
        )
        ...
        if "improved_locks" in self.results:
            ...
            print(
                f"     • Locks exitosos: {locks_data['successful_locks']}/10 clientes"
            )
            print(
                f"     • Tiempo promedio: {locks_data['avg_acquisition_time'] * 1000:.1f}ms"
            )
        ...
        if "health_monitoring" in self.results:
            ...
            print(
                f"     • Database: {'✅' if health_data['database_available'] else '❌'}"
            )
