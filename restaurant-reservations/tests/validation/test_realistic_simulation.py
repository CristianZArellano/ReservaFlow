...
class RealisticTestEnvironment:
    ...
    def setup_celery_simulation(self):
        ...
        def simulate_async_task(task_func, task_id, args=None, kwargs=None):
            ...
            return {"task_id": task_id, "status": "PENDING", "thread": thread}
    ...
    def test_concurrent_reservations_with_locks(self):
        ...
        for customer_id in customers:
            thread = threading.Thread(
                target=try_create_reservation, args=(customer_id,)
            )
        ...
        print(
            f"  {'✅' if success else '❌'} Concurrencia manejada correctamente: {success}"
        )
...
class RealisticCeleryTaskTest:
    ...
    def test_expire_reservation_task(self):
        ...
        task_info = self.env.simulate_async_task(
            expire_reservation, task_id, args=[str(reservation.id)]
        )
    ...
    def test_email_confirmation_task(self):
        ...
        with patch("django.core.mail.send_mail") as mock_send_mail:
            ...
            task_info = self.env.simulate_async_task(
                send_confirmation_email, task_id, args=[str(reservation.id)]
            )
...
def run_realistic_tests():
    ...
    try:
        ...
        test_results["redis_concurrency"] = (
            redis_test.test_concurrent_reservations_with_locks()
        )
    except Exception as e:
    ...
    if redis_real:
        ...
        print(
            "  🔴 Locks simulados pueden no detectar todas las condiciones de carrera"
        )
