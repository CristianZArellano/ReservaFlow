        ...
        try:
            ...
            try:
                with TableReservationLock(
                    table.id, "2025-09-25", "18:00", timeout=60, max_retries=3
                ) as lock:
                    acquisition_time = time.time() - start_time
                    print(
                        f"  ✅ Lock adquirido con retry ({acquisition_time * 1000:.1f}ms)"
                    )
    ...
    def test_security_configurations(self):
        ...
        try:
            ...
            if (
                ...
                and "DEFAULT_THROTTLE_RATES" in settings.REST_FRAMEWORK
            ):
        ...
        try:
            ...
            try:
                available = check_table_availability(
                    table.id, "2025-09-26", "19:00", use_cache=True
                )
            ...
            optimization_score = sum(
                [cache_working, query_optimized, connection_healthy]
            )
    ...
    def test_integration_robustness(self):
        ...
        try:
            ...
            def complete_reservation_flow(customer):
                try:
                    # 1. Acquire lock
                    with TableReservationLock(table.id, "2025-09-27", "20:00"):
                        ...
                        with transaction.atomic():
                            ...
                            results.append(
                                f"✅ Customer {customer.id}: Email programado"
                            )
            ...
            for customer in customers:
                thread = threading.Thread(
                    target=complete_reservation_flow, args=(customer,)
                )
