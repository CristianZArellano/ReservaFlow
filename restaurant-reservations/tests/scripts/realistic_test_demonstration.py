...
class RealisticTestDemonstration:
    ...
    def demonstrate_redis_locks(self):
        ...
        def try_acquire_lock_realistic(client_id):
            ...
            try:
                ...
                actual_latency = base_latency + random.uniform(
                    -network_jitter / 1000, network_jitter / 1000
                )
                ...
                if redis_cpu_load > 0.85 and random.random() < 0.15:
                    timeout_errors.append(
                        f"Client {client_id}: Redis server overloaded"
                    )
                    return
                ...
                if lock_acquired:
                    results.append(
                        f"Client {client_id}: LOCK ACQUIRED ({actual_latency * 1000:.1f}ms)"
                    )
                    ...
                    if (
                        lock_key in self.redis_store
                        and self.redis_store[lock_key]["owner"] == client_id
                    ):
                        ...
                        if datetime.now() < self.redis_store[lock_key]["expires_at"]:
                            ...
                            errors.append(
                                f"Client {client_id}: Lock expired during work"
                            )
                    else:
                    ...
                    if lock_key in self.redis_store:
                        ...
                        errors.append(
                            f"Client {client_id}: Lock held by client {lock_info['owner']} for {time_held:.2f}s"
                        )
                    else:
                        errors.append(
                            f"Client {client_id}: Race condition - lock taken during check"
                        )
        ...
        total_attempts = (
            len(results) + len(errors) + len(timeout_errors) + len(network_errors)
        )

        print(f"\n  📊 Resultados de {total_attempts} intentos concurrentes:")
        print(
            f"     ✅ Locks exitosos: {len(results)} ({len(results) / total_attempts:.1%})"
        )
        print(
            f"     ⏰ Timeouts/Overload: {len(timeout_errors)} ({len(timeout_errors) / total_attempts:.1%})"
        )
        print(
            f"     🌐 Errores de red: {len(network_errors)} ({len(network_errors) / total_attempts:.1%})"
        )
        print(
            f"     ❌ Contención/Race: {len(errors)} ({len(errors) / total_attempts:.1%})"
        )
        ...
        realistic_behavior = (
            success_rate <= 0.25
        )  # Máximo 25% éxito es realista con alta contención
    ...
    def demonstrate_celery_tasks(self):
            ...
            def execute_async():
                ...
                failure_scenarios = [
                    (
                        0.25,
                        "SMTP server timeout",
                        f"Network delay: {network_delay:.2f}s",
                    ),
                    (
                        0.15,
                        "Broker connection lost",
                        f"Broker health: {broker_health:.2f}",
                    ),
                    (
                        0.20,
                        "Email service rate limit",
                        f"SMTP load: {smtp_server_load:.2f}",
                    ),
        ...
        for i in range(5):  # Más tareas para mejor estadística
            task_id, thread = simulate_email_task(
                f"res_{i}", f"customer{i}@email.com", i
            )
        ...
        for task_id, thread in tasks:
            ...
            if result["status"] == "SUCCESS":
                print(
                    f"  {status_icon} {task_id}: {result['status']} ({result.get('delivery_time', 'N/A')})"
                )
            else:
                print(
                    f"  {status_icon} {task_id}: {result['status']} - {result['error']}"
                )
        ...
        success_count = sum(
            ...
            if self.celery_tasks[task_id]["status"] == "SUCCESS"
        )
        failure_count = len(tasks) - success_count

        print("\n  📊 Resultados realistas:")
        print(
            f"     ✅ Exitosas: {success_count}/{len(tasks)} ({success_count / len(tasks):.1%})"
        )
        print(
            f"     ❌ Fallidas: {failure_count}/{len(tasks)} ({failure_count / len(tasks):.1%})"
        )
        ...
        self.results["celery_tasks"] = {
            ...
            > 0,  # Algunos fallos son esperados en condiciones reales
            "mock_issues": [
                "Always synchronous",
                "No network failures",
                "No broker issues",
    ...
    def demonstrate_integration_scenarios(self):
        ...
        def complete_reservation_flow_realistic(
            customer_id, table_id, email, attempt_delay=0
        ):
            ...
            try:
                ...
                while time.time() - start_time < lock_timeout:
                    if lock_key not in self.redis_store:
                        ...
                        if lock_key not in self.redis_store:
                            ...
                            lock_acquired = True
                            steps.append(
                                f"✅ Lock acquired after {(time.time() - start_time) * 1000:.0f}ms"
                            )
                            break
                ...
                if emails_sent_count >= email_service_quota:
                    steps.append(
                        "⚠️ Email quota exceeded - reservation confirmed but no email"
                    )
                    del self.redis_store[lock_key]
                    return steps
        ...
        def run_flow(customer_id):
            ...
            result = complete_reservation_flow_realistic(
                customer_id, "table_premium", f"customer{customer_id}@email.com", delay
            )
    ...
    def generate_final_report(self):
        ...
        realistic_passed = sum(
            1 for result in self.results.values() if result["realistic"]
        )
