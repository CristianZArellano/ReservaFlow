...
def wait_for_services():
    ...
    services_to_check = [
        ('PostgreSQL', 'docker compose -f docker-compose.test.yml exec -T test-db pg_isready -U test_user -d reservaflow_test'),
        ('Redis', 'docker compose -f docker-compose.test.yml exec -T test-redis redis-cli ping'),
    ]
    ...
    for service_name, check_cmd in services_to_check:
        ...
        for i in range(max_retries):
            if run_command(check_cmd, check=False):
                ...
                break
            print(f"⏳ {service_name} no está listo, reintentando ({i+1}/{max_retries})...")
            time.sleep(2)
        else:
            print(f"❌ {service_name} no se pudo conectar después de {max_retries} intentos")
            return False
...
def setup_test_environment():
    ...
    if not run_command("docker compose -f docker-compose.test.yml exec -T test-web python manage.py migrate --settings=config.settings_test"):
        return False
...
def run_tests(test_path=None, verbose=False, failfast=False):
    ...
    env_vars = [
        ...
        "-e", "PYTHONPATH=/app",
        "-e", "CELERY_TASK_ALWAYS_EAGER=False",  # CRÍTICO: Tests realistas
    ]
...
def show_service_status():
    ...
    print("\n🔴 REDIS INFO:")
    run_command("docker compose -f docker-compose.test.yml exec -T test-redis redis-cli info server", check=False)
    
    print("\n📊 POSTGRESQL STATUS:")
    run_command("docker compose -f docker-compose.test.yml exec -T test-db psql -U test_user -d reservaflow_test -c 'SELECT COUNT(*) as reservations_count FROM reservations_reservation;'", check=False)

def main():
    parser = argparse.ArgumentParser(description="Ejecutar tests realistas con Docker")
    parser.add_argument('--setup-only', action='store_true', help='Solo configurar entorno sin ejecutar tests')
    parser.add_argument('--cleanup', action='store_true', help='Solo limpiar servicios Docker')
    parser.add_argument('--status', action='store_true', help='Mostrar estado de servicios')
    parser.add_argument('--test-path', type=str, help='Path específico de tests a ejecutar')
    parser.add_argument('--categories', action='store_true', help='Ejecutar tests por categorías')
    parser.add_argument('--verbose', '-v', action='store_true', help='Output verboso')
    parser.add_argument('--failfast', '-x', action='store_true', help='Parar en primer fallo')
