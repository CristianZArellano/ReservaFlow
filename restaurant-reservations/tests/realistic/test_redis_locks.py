...
class RedisLockRealisticTest(TransactionTestCase):
    ...
    def test_redis_lock_acquisition(self):
        ...
        lock_key = f"table_lock:{self.table.id}:{self.test_date}:{self.test_time}"
    ...
    def test_concurrent_reservation_with_real_locks(self):
        ...
        self.assertEqual(
            len(errors), 4,
            f"Debe haber 4 errores, pero hubo {len(errors)}: {errors}"
        )
...
class RedisAvailabilityCheckTest(TestCase):
    ...
    def test_availability_caching(self):
        ...
        print(f"Disponibilidad después de crear reserva: {available2} (puede ser True por cache)")
        ...
        self.assertFalse(available3, "Después de invalidar cache debe reflejar reserva existente")
