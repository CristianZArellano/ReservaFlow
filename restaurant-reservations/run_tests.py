...
def run_command(command, description, capture_output=False):
    ...
    try:
        if capture_output:
            ...
            result = subprocess.run(
                command, shell=True, cwd=os.path.dirname(os.path.abspath(__file__))
            )
...
def run_unit_tests():
    ...
    success, stdout, stderr = run_command(
        "python -m pytest tests/unit/ -v --tb=short", "Ejecutando Tests Unitarios"
    )
    return success
...
def run_realistic_tests():
    ...
    success, stdout, stderr = run_command(
        "python -m pytest tests/realistic/ -v --tb=short", "Ejecutando Tests Realistas"
    )
    return success
...
def run_validation_tests():
    ...
    validation_tests = [
        ...
        (
            "tests/validation/test_all_problems_fixed.py",
            "All Problems Fixed Validation",
        ),
    ]
    ...
    for test_file, description in validation_tests:
        success, stdout, stderr = run_command(
            f"python {test_file}", f"🧪 {description}"
        )
...
def run_realistic_demonstrations():
    ...
    demonstrations = [
        (
            "tests/scripts/realistic_test_demonstration.py",
            "Realistic Test Demonstration",
        ),
    ...
    for script_file, description in demonstrations:
        success, stdout, stderr = run_command(
            f"python {script_file}", f"🎯 {description}"
        )
        all_passed = all_passed and success
        if not success:
            print(f"   ❌ Falló: {description}")
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Ejecutar solo tests rápidos"
    )
